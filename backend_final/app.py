...
def get_events_report():
    ...
    for e in my_events:
        ...
        report.append({
            ...
            "revenue": revenue,
            "image": e.image,
            "status": "active" if e.event_timestamp > datetime.datetime.now(datetime.timezone.utc).timestamp() * 1000 else "finished"
        })
...
def handle_events():
    if request.method == 'POST':
        ...
        new_event = Event(
            ...
            price=float(data.get('priceValue', 0)),
            price_value=float(data.get('priceValue', 0)),
            location=data.get('location', ''), image=data.get('image', ''),
            event_timestamp=data.get('timestamp', int(datetime.datetime.now(datetime.timezone.utc).timestamp() * 1000)),
...
def handle_single_event(event_id):
    ...
    if request.method == 'PUT':
        ...
        event.price = data.get('priceValue', event.price)  # синхронизация price
...
def buy_ticket():
    ...
    new_ticket = Ticket(
        event_id=data['eventId'], user_id=uid, quantity=data.get('quantity', 1),
...
def sync_with_1c():
    try:
        approved_ids_from_1c = ODataService.get_approved_event_ids()
...
# ==========================================
# ШЛЮЗ ДЛЯ 1С (API GATEWAY)
# ==========================================

SYNC_API_KEY = os.environ.get('SYNC_API_KEY', 'Secret_Key_For_Eventum_1C_Integration')
...
# --- Вспомогательные функции (чистые данные) ---
...
def _get_unsynced_events():
    ...
    for e in events:
        date_val = ""
        if hasattr(e, 'event_timestamp') and e.event_timestamp:
