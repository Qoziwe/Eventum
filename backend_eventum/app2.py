...
from services.odata_service import ODataService
...
def upload_avatar():
    ...
    if file and is_safe_file(file):
        ...
        new_filename = f"avatar_{user_id}_{int(datetime.datetime.now(datetime.timezone.utc).timestamp())}.{ext}"
...
def upload_event_image():
    ...
    if file and is_safe_file(file):
        user_id = get_jwt_identity()
        ext = file.filename.rsplit('.', 1)[1].lower()
        new_filename = f"event_{user_id}_{int(datetime.datetime.now(datetime.timezone.utc).timestamp())}.{ext}"
        new_filename = secure_filename(new_filename)
...
class Notification(db.Model):
    ...
    def to_dict(self):
        ts_str = self.timestamp.isoformat() if self.timestamp else datetime.datetime.now(datetime.timezone.utc).isoformat()
        return {
            "id": self.id,
...
def on_private_message(data):
    ...
    if recipient_active_target != sender_id:
        ...
        notif_id = f"notif_{int(datetime.datetime.now(datetime.timezone.utc).timestamp() * 1000)}_{recipient_id}"
...
def register():
    ...
    new_user = User(
        ...
        user_type=data.get('userType', 'explorer'),
        birth_date=data.get('birthDate', '2000-01-01'), 
        location=data.get('location', 'Алматы'),
        
        # НОВОЕ: Ставим флаг, что в 1С еще не отправлено
        is_synced=False
    )
    ...
    # Блок ODataService УДАЛЕН. 1С заберет пользователя позже сама.
...
def handle_events():
    ...
    if request.method == 'POST':
        ...
        new_event = Event(
            ...
            organizer_avatar=data.get('organizerAvatar', ''),
            ...
            # НОВОЕ: Сохраняем цену и статус синхронизации
            price=float(data.get('priceValue', 0)), # Пишем в новую колонку price
            price_value=float(data.get('priceValue', 0)), # Дублируем в старую для совместимости
            is_synced=False
        )
    ...
    # --- ПОЛУЧЕНИЕ (GET) ---
    # (Этот блок оставляем без изменений, только чтение)
    user_id = get_jwt_identity()
    if user_id:
    ...
    for e in events:
        ...
        result.append({
            ...
            "categories": e.categories, "priceValue": e.price, "location": e.location, 
...
def increment_event_view(event_id):
    try:
        ...
        last_minute = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=1)
...
def handle_posts():
    ...
    return jsonify([{
        ...
        "upvotes": p.upvotes or 0, "downvotes": p.downvotes or 0, "ageLimit": p.age_limit, 
        "timestamp": p.timestamp.isoformat(), "commentCount": len(p.comments), 
        "votedUsers": {v.user_id: v.vote_type for v in p.votes}, 
        "moderationStatus": p.moderation_status, "rejectionReason": p.rejection_reason
    } for p in posts])
...
def buy_ticket():
    ...
    new_ticket = Ticket(
        ...
        # НОВОЕ: Фиксируем цену покупки (чтобы знать выручку)
        purchase_price=event.price,
        
        # НОВОЕ: Ставим флаг, что в 1С еще не ушло
        is_synced=False
    )
    ...
    # Блок ODataService УДАЛЕН. 
    # Теперь никаких тормозов при покупке.

    return jsonify({"message": "OK", "ticketId": new_ticket.id}), 201
...
@app.route('/api/admin/sync-1c', methods=['POST'])
@admin_required
def sync_with_1c():
    try:
        # 1. Получаем список ID одобренных ивентов из 1С
        approved_ids_from_1c = ODataService.get_approved_events_ids()
        
        if not approved_ids_from_1c:
            return jsonify({"message": "Нет одобренных мероприятий в 1С или ошибка связи"}), 200

        # 2. Находим у нас ивенты, которые 'pending', но в 1С уже одобрены
        count_updated = 0
        events_to_update = Event.query.filter(
            Event.id.in_(approved_ids_from_1c),
            Event.moderation_status == 'pending'
        ).all()

        # 3. Обновляем их статус
        for event in events_to_update:
            event.moderation_status = 'approved'
            event.rejection_reason = None
            count_updated += 1
            
            # (Опционально) Уведомляем автора, что его ивент одобрен
            # ... тут можно добавить код уведомления, как в admin_moderate_event ...

        db.session.commit()
        
        return jsonify({
            "message": "Синхронизация успешна", 
            "updatedCount": count_updated,
            "approvedIn1C": len(approved_ids_from_1c)
        }), 200
...
# ... где-то в конце файла, рядом с другими админскими путями ...

@app.route('/api/admin/manual-sync', methods=['GET'])
def manual_sync_1c():
    """
    ВОЛШЕБНАЯ ССЫЛКА: Открой в браузере, чтобы подтянуть одобрения из 1С.
    """
    try:
        print("[Sync] Начинаем ручную синхронизацию...")
        
        # 1. Спрашиваем у 1С список ID одобренных (через твой сервис)
        approved_ids = ODataService.get_approved_event_ids()
        
        if not approved_ids:
            return "<h3>Синхронизация завершена</h3><p>В 1С нет новых одобренных событий (или 1С недоступна).</p>", 200

        # 2. Ищем у нас события, которые 'pending', но в 1С уже с галочкой
        events_to_update = Event.query.filter(
            Event.id.in_(approved_ids),
            Event.moderation_status == 'pending'
        ).all()

        if not events_to_update:
            return f"<h3>Синхронизация завершена</h3><p>Найдено одобренных в 1С: {len(approved_ids)}, но они уже обновлены на сайте.</p>", 200
        ...
        # Красивый ответ в браузер
        return f"""
        <h1>УСПЕШНО! ✅</h1>
        <p>Синхронизировано событий: <b>{count}</b></p>
        <ul>
            {''.join([f'<li>{t}</li>' for t in updated_titles])}
        </ul>
        <br>
        <button onclick="window.location.reload()">Синхронизировать еще раз</button>
        """, 200
...
# ==========================================
# ПРАВИЛЬНЫЙ ШЛЮЗ ДЛЯ 1С (API GATEWAY) v2
# ==========================================

SYNC_API_KEY = "Secret_Key_For_Eventum_1C_Integration"

def check_api_key():
    """Охранник: проверяет ключ"""
    key = request.headers.get('X-API-KEY')
    if key != SYNC_API_KEY:
        return False
    return True

# --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (Возвращают чистые данные) ---
# Это нужно, чтобы не было ошибки 500 при вызове "ВСЁ СРАЗУ"

def _get_unsynced_users():
    users = User.query.filter_by(is_synced=False).all()
    return [
        {
            "id": u.id, 
            "nickname": u.name or u.username, 
            "email": u.email,
            # Добавляем пустую строку, если телефона нет, чтобы 1С не ругалась
            "phone": u.phone if hasattr(u, 'phone') and u.phone else "" 
        } for u in users
    ]

def _get_unsynced_events():
    events = Event.query.filter_by(is_synced=False).all()
    data = []
    for e in events:
        # Безопасное получение даты
        date_val = ""
        if hasattr(e, 'date') and e.date:
            date_val = e.date.isoformat()
        elif hasattr(e, 'event_timestamp') and e.event_timestamp:
             # Если у тебя timestamp в миллисекундах
            date_val = datetime.datetime.fromtimestamp(e.event_timestamp / 1000).isoformat()

        data.append({
            "id": e.id, 
            "title": e.title,
            "creator_id": e.organizer_id,
            "price": e.price if hasattr(e, 'price') else 0,
            "date": date_val
        })
    return data

def _get_unsynced_tickets():
    tickets = Ticket.query.filter_by(is_synced=False).all()
    result = []
    for t in tickets:
        event = db.session.get(Event, t.event_id)
        
        # Безопасное получение даты покупки
        purchase_date = ""
        if hasattr(t, 'purchase_date') and t.purchase_date:
        ...
        result.append({
            ...
            "price": t.purchase_price if hasattr(t, 'purchase_price') else 0,
            "quantity": t.quantity,
            "date": purchase_date
        })
    return result
...
@app.route('/api/v1/sync/users', methods=['GET'])
def sync_users_only():
    if not check_api_key(): return jsonify({"error": "Unauthorized"}), 401
    return jsonify(_get_unsynced_users())

@app.route('/api/v1/sync/events', methods=['GET'])
def sync_events_only():
    if not check_api_key(): return jsonify({"error": "Unauthorized"}), 401
    return jsonify(_get_unsynced_events())

@app.route('/api/v1/sync/tickets', methods=['GET'])
def sync_tickets_only():
    if not check_api_key(): return jsonify({"error": "Unauthorized"}), 401
    return jsonify(_get_unsynced_tickets())

@app.route('/api/v1/sync/all', methods=['GET'])
def sync_all_data():
    if not check_api_key(): return jsonify({"error": "Unauthorized"}), 401
    
    # Теперь мы вызываем функции, а не маршруты -> ОШИБКИ 500 НЕ БУДЕТ
    return jsonify({
        "users": _get_unsynced_users(),
        "events": _get_unsynced_events(),
        "tickets": _get_unsynced_tickets()
    })

@app.route('/api/v1/sync/confirm', methods=['POST'])
def confirm_sync_success():
    # 1. Проверяем ключ
    if not check_api_key(): 
        return jsonify({"error": "Unauthorized"}), 401
    
    # 2. Получаем данные (force=True)
    data = request.get_json(force=True, silent=True)
    
    if not data:
        print("❌ POST пришел без данных")
        return jsonify({"error": "No JSON data received"}), 400

    print(f"📥 1C CONFIRM REQUEST: {data}") 
    
    try:
        updated_counts = {"users": 0, "events": 0, "tickets": 0}

        # === USERS (Работаем со строками!) ===
        if 'users' in data and data['users']:
            # Просто берем список ID как есть, без int()
            user_ids = data['users']
            if user_ids:
                rows = User.query.filter(User.id.in_(user_ids)).update({User.is_synced: True}, synchronize_session=False)
                updated_counts["users"] = rows

        # === EVENTS (Работаем со строками!) ===
        if 'events' in data and data['events']:
            event_ids = data['events']
            if event_ids:
                rows = Event.query.filter(Event.id.in_(event_ids)).update({Event.is_synced: True}, synchronize_session=False)
                updated_counts["events"] = rows
            
        # === TICKETS (Работаем со строками!) ===
        if 'tickets' in data and data['tickets']:
            ticket_ids = data['tickets']
            if ticket_ids:
                rows = Ticket.query.filter(Ticket.id.in_(ticket_ids)).update({Ticket.is_synced: True}, synchronize_session=False)
                updated_counts["tickets"] = rows

        db.session.commit()
        
        print(f"✅ УСПЕХ: Статус обновлен. {updated_counts}")
        return jsonify({"status": "confirmed", "updated": updated_counts}), 200

    except Exception as e:
        db.session.rollback()
        print(f"❌ ОШИБКА ВНУТРИ CONFIRM: {e}")
        return jsonify({"error": str(e)}), 500




    # --- РОУТ ДЛЯ ОДОБРЕНИЯ ИВЕНТА ИЗ 1С ---
@app.route('/api/v1/sync/update_event_status', methods=['POST'])
def update_event_status():
    ...
    target_id = str(data['event_id'])
    # Получаем желаемый статус (по умолчанию 'approved', если 1С забыла прислать)
    new_status = data.get('status', 'approved') 
    # Получаем причину (если есть)
    reason_text = data.get('reason', None)

    print(f"🔄 1С меняет статус ивента {target_id} на: {new_status}")

    try:
        event = Event.query.filter_by(id=target_id).first()
        
        if not event:
            print(f"❌ Ивент {target_id} не найден!")
            return jsonify({"error": "Event not found"}), 404
        ...
        if new_status == 'rejected':
            # 1. Если ОТКАЗ
            event.moderation_status = 'rejected'
            event.rejection_reason = reason_text
            print(f"⛔ Ивент отклонен. Причина: {reason_text}")
            
        elif new_status == 'approved':
            # 2. Если ОДОБРЕНИЕ
            event.moderation_status = 'approved'
            event.rejection_reason = None # Очищаем старую причину отказа, если была
            print(f"✅ Ивент одобрен.")
            
        else:
            return jsonify({"error": "Invalid status provided"}), 400

        db.session.commit()
        return jsonify({"status": new_status, "id": target_id}), 200
