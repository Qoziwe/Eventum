...
class User(db.Model):
    ...
    # НОВОЕ: Статус синхронизации с 1С
    ...
    # СВЯЗЬ С БИЛЕТАМИ (Важно для аналитики)
    # Позволяет писать user.tickets и получать всю историю покупок
...
class Event(db.Model):
    ...
    # НОВОЕ: Цена и Синхронизация
    price_value = db.Column(db.Float, default=0.0) # Старое поле (оставил, чтобы не терять данные)
...
class Ticket(db.Model):
    ...
    # НОВОЕ: Финансовая история и синхронизация
    ...
    # ВАЖНО: Я УБРАЛ UniqueConstraint, чтобы пользователь мог купить билет на один и тот же ивент несколько раз!
