...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | /api/events | List approved events, filtered by city (+ own pending for organizers) |
| POST | /api/events | Create event with city association (auto-sent to moderation) |
...
| GET | /api/chat/<user_id> | Get chat history (attachment support removed) |
...
- CSRF/CSWH Protection - WebSocket origin validation restricted to configured allowed origins via .env
...
| Model | Purpose |
|-------|---------|
| User | User accounts with roles, city, banning, following |
| Event | Events with city location, moderation, views, pricing |
