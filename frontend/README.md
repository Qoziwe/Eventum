...
|   |   |-- Header.tsx              # Universal header with notifications & city selection
...
|-----|--------|-------------|
| Home | HomeScreen | Event feed with hero carousel, optimized transitions, and city-based filters |
| Search | SearchScreen | Full-text event & user search with performance tuning |
...
| Screen | Description |
|--------|-------------|
| EventDetailScreen | Full event view with ticket purchase, sharing, and native Maps deep-linking |
| CreateEventScreen | Rich event creation with dynamic 2-year calendar, 60-min time picker, city dropdown, and image upload |
...
Eight Zustand stores manage application state:
...
| configStore | City selection, dynamic platform configuration |
...
- Smooth Tab Navigation: Heavy rendering tasks are deferred via InteractionManager to guarantee 60 FPS transitions.
